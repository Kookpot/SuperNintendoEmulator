namespace SuperNintendo.Core.DMA
{
    public class DMA
    {
        public static readonly SDMA[] SDMA = new SDMA[8];

        public static void ResetDMA()
        {
            for (var d = 0; d < 8; d++)
                SDMA[d] = new SDMA();
        }
        ...
        {
            CPU.CPUState.Cycles += n;
        }

        //private static bool AddCyclesInDMA(byte dmaChannel)
        //{
        ...
        //    AddCycles(CPU.Constants.SLOW_ONE_CYCLE);
        //    while (CPU.CPUState.Cycles >= CPU.CPUState.NextEvent)
        //        CPU.CPU.DoHEventProcessing();

        //    if ((CPU.CPUState.HDMARanInDMA & (1 << dmaChannel)) > 0)
        //    {
        //        CPU.CPUState.HDMARanInDMA = 0;
...
//        private static bool DoDMA(byte channel)
//        {
//            CPU.CPUState.InDMA = true;
//            CPU.CPUState.InDMAorHDMA = true;
//            CPU.CPUState.CurrentDMAorHDMAChannel = channel;
...
//                int c = d.DMACount_Or_HDMAIndirectAddress;
...
//                    {
//                        CPU.CPUState.InDMA = false;
//                        CPU.CPUState.InDMAorHDMA = false;
//                        CPU.CPUState.CurrentDMAorHDMAChannel = -1;
//                        return false;
//                    }
//                }
...
//                        PPU.IPPU.FlushRedraw();
//                    break;
//            }

//            var inc = d.AAddressFixed ? 0 : (!d.AAddressDecrement ? 1 : -1);
//            int count = d.DMACount_Or_HDMAIndirectAddress;
//            // Writing $0000 to $43x5 actually results in a transfer of $10000 bytes, not 0.
//            if (count == 0)
//                count = 0x10000;
...
//            int? in_sdd1_dma = null;

//            if (Settings.SDD1)
//            {
//                if (d.AAddressFixed && Memory.Memory.ROM[Memory.Memory.FillRAMPosition + 0x4801] > 0)
//                {
...
//                    int? in_ptr = Memory.Memory.GetBasePointer((uint) ((d.ABank << 16) | d.AAddress));
//                    if (in_ptr != null)
//                    {
//                        in_ptr += d.AAddress;
//                        SDD1_decompress(sdd1_decode_buffer, in_ptr, d.DMACount_Or_HDMAIndirectAddress);
//                    }

//                    in_sdd1_dma = sdd1_decode_buffer;
//                }

//                Memory.Memory.ROM[Memory.Memory.FillRAMPosition + 0x4801] = 0;
//            }
...
//                {
//                    spc7110_dma = new byte[d.DMACount_Or_HDMAIndirectAddress];
//                    for (int i = 0; i < d.DMACount_Or_HDMAIndirectAddress; i++)
//                        spc7110_dma[i] = s7emu.decomp.read();

//                    int icount = s7emu.r4809 | (s7emu.r480a << 8);
//                    icount -= d.DMACount_Or_HDMAIndirectAddress;
...
//        public static byte DoHDMA(byte data)
//        {
